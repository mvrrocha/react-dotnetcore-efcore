...
export default function AtividadeItem(props) {
  ...
  function prioridadeStyle(param, icone) {
    switch (param) {
      case 'Baixa': return icone ? 'smile' : 'success';
      case 'Normal': return icone ? 'meh' : 'dark';
      case 'Alta': return icone ? 'frown' : 'warning';
      ...
      <div className="card-body">
        ...
        <div className='d-flex justify-content-end border-top pt-2 m-0'>
          <button className='btn btn-outline-primary me-2 btn-sm'
            ...
            <i className='fa fa-pen me-2'></i>Editar</button>
          <button className='btn btn-outline-danger btn-sm' 
                  onClick={() => props.handleConfirmModal(props.ativ.id)}>
            <i className='fa fa-trash me-2'></i>Deletar</button>
        </div>
      </div>
    </div>
  )
